<template>
          <div class="col-lg-8 grid-margin stretch-card" >
            <div class="card">
              <div class="card-body">
                <h4 class="card-title">Messaging and branding</h4>
                <p class="card-description">
                  Use navigation at the top | <span class="text-success">Use actions column for each</span>
                </p>
                <input type="text" placeholder="Search sku name here.." class="form-control" style="width: 300px;" v-model="searchTerm"> 
                  ...
                  <table class="table table-striped">
                    ...
                    <tbody>
                      <tr v-for="item in filtersearch" :key="item.id">
                        ...
                        <td >
                          <router-link :to="{ name: 'edit-tm-message' , params:{id:item.id} }" class="btn btn-primary btn-xs" >Edit</router-link>
  ...
  computed:{
      filtersearch(){
          return this.items.filter(item =>{
              return item.sku_name.match(this.searchTerm)
          })
      }
  },
  methods:{
      allItems(){
        let id = localStorage.getItem('company_name')
          axios.get('/api/viewtmmessage/'+id)
      ...
      deleteItem(id){
          Swal.fire({
              ...
              if (result.isConfirmed) {
                      // Pass your code here 
                  axios.delete('/api/deletetmmessage/'+id)
                  .then(()=>{
